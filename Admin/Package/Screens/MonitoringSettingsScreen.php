...
class MonitoringSettingsScreen extends Screen
{
    ...
    public function mount(): void
    {
        ...
        breadcrumb()->add(__('def.admin_panel'), url('/admin'))->add(__('monitoring.admin.title'));
    }
    ...
    public function commandBar(): array
    {
        return [
            ...
            Button::make(__('def.save'))->icon('ph.regular.floppy-disk')->method('save'),
        ];
    }
        ...
        return [
            LayoutFactory::columns([
                LayoutFactory::blank([
                    LayoutFactory::block([
                        LayoutFactory::field(
                            Input::make('connection_timeout')
                                ->type('number')
                                ->value(request()->input('connection_timeout', config(
                                    'monitoring.connection_timeout',
                                    3,
                                )))
                                ->min(1)
                                ->max(30),
                        ...
                        LayoutFactory::field(
                            Input::make('status_retention_days')
                                ->type('number')
                                ->value(request()->input('status_retention_days', config(
                                    'monitoring.status_retention_days',
                                    60,
                                )))
                                ->min(7)
                                ->max(365),
                    ...
                    LayoutFactory::block([
                        LayoutFactory::field(Toggle::make('show_navbar_online')->checked(filter_var(
                            request()->input('show_navbar_online', config('monitoring.show_navbar_online', true)),
                            FILTER_VALIDATE_BOOLEAN,
                        )))
                            ->label(__('monitoring.admin.show_navbar_online'))
                            ->popover(__('monitoring.admin.show_navbar_online_help')),

                        LayoutFactory::field(Toggle::make('show_offline_servers')->checked(filter_var(
                            request()->input('show_offline_servers', config('monitoring.show_offline_servers', true)),
                            FILTER_VALIDATE_BOOLEAN,
                        )))
                            ->label(__('monitoring.admin.show_offline_servers'))
                            ->popover(__('monitoring.admin.show_offline_servers_help')),

                        LayoutFactory::field(Toggle::make('show_player_details')->checked(filter_var(
                            request()->input('show_player_details', config('monitoring.show_player_details', true)),
                            FILTER_VALIDATE_BOOLEAN,
                        )))
                ...
                LayoutFactory::blank([
                    LayoutFactory::block([
                        LayoutFactory::field(
                            Select::make('default_display_mode')
                                ...
                                ->aligned()
                                ->value(request()->input('default_display_mode', config(
                                    'monitoring.default_display_mode',
                                    'standard',
                                ))),
                        )
                        ...
                        LayoutFactory::field(
                            Input::make('default_servers_limit')
                                ->type('number')
                                ->value(request()->input('default_servers_limit', config(
                                    'monitoring.default_servers_limit',
                                    50,
                                )))
                                ->min(1)
                                ->max(200),
                    ...
                    LayoutFactory::block([
                        LayoutFactory::field(Toggle::make('faceit_enabled')->checked(filter_var(
                            request()->input('faceit_enabled', config('monitoring.faceit.enabled', false)),
                            FILTER_VALIDATE_BOOLEAN,
                        )))
